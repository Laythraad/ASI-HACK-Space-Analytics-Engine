...
def build_content(D: dict, files: list) -> list:
    ...
    SEC.append(("2. كيف تقيّم المشروع خلال خمس دقائق", [
        ...
        tab(["#", "الإجراء", "النتيجة المتوقعة"], [
            ...
            ["2", "افتح http://127.0.0.1:5000/mors",
             "واجهة MORS: شريط صحة البيانات + 18 وحدة في الشريط الجانبي"],
        ...
        p("المشروع يعمل من دون مفاتيح بالوضع المحلي الاحتياطي (local_model)، "
          ...
          "الحتمي ويسجَّل هذا التحويل في council.engine وفي مشكلة P-001 — "
    ...
    for f in files:
        inv_rows.append([f["path"], kb(f["bytes"]),
                         roles.get(f["path"]) or "-"])
    SEC.append(("3. محتويات الحزمة — كل ملف وما يفعله", [
        p("القائمة بترتيب القراءة المقدَّم: المستندات المُصدَّرة أولًا، ثم نقطة "
          "الدخول، ثم الوكيلون، ثم الواجهات، ثم أدوات التوليد، ثم المستندات."),
        tab(["الملف", "الحجم", "الدور"], inv_rows, [58, 16, 96]),
        p(f"الإجمالي {len(files)} ملفًا بحجم "
          f"{sum(f['bytes'] for f in files) / 1024:.0f} كيلوبايت (قبل الضغط)."),
    ]))
    ...
    SEC.append(("10. حدود صادقة — ما لا ندّعي إنجازه", [
        ...
        tab(["الرمز", "المشكلة", "الأولوية", "الثقة"], pr_rows,
            [16, 96, 26, 32]),
        bul([
            "P-001: تحقّق المجلس العلمي يعمل بالمحرك الاحتياطي المحلي عند "
            "نفاد حصيلة Gemini المجانية؛ الأرقام نفسها تبقى قابلة للتكرار "
            "لكنها غير مُتقاطعة من نموذج لغوي.",
            "P-006: الملفات متعددة الطيف مبنية على طيفات مرجعية لا مشاهد "
            "مُصنَّفة فعليًا — والمصدر مذكور صراحةً.",
            "P-003: عدد الشواذ (167) مشتقّ من معامل تلوث ثابت 0.1 لا من "
...
def render_md(sections, meta) -> str:
    out = [
        ...
        f"- **عدد الملفات في الحزمة:** {meta['files']}",
        "",
